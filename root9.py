...
def show_board(puzzle, solution):
    ...
    def check_cell(event, row, col):
        ...
        if int(val) != solution[row][col]:
            ...
            cells[row][col].config(bg='#c9f7c9')
    ...
    def give_hint():
        ...
        cells[row][col].config(state="readonly", readonlybackground="#c9f7c9")
...
def open_settings():
    ...
    error_entry = tk.Entry(settings_window)
    error_entry.pack()
...
tk.Button(root, text="Начать игру", command=start_game).pack(pady=10)

tk.Button(root, text="Настройки",command=open_settings).pack(pady=10)

tk.Button(root, text="Правила").pack(pady=10)
